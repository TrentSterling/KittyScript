...
function GM:PlayerSay( ply, text )
	...
	if( 0 == 0 ) then--alltalk is 0?
		RangeChat( ply:Nick() .. ": " .. text, ply:GetPos(), 250 , ply );
		return "";
	end
	return text;
end
...
function Yell( ply, args )
	RangeChat( "[YELL]" .. ply:Nick() .. ": " .. args, ply:GetPos(), 550 , ply);
	return "";
end
...
function Boom( ply, args )
	...
	effectdata:SetScale(1.5)
	util.Effect("HelicopterMegaBomb", effectdata)
	RangeChatKill( "[YELL]" .. ply:Nick() .. ": " .. args, ply:GetPos(), 550 , ply);
	return "";
end
...
function Me( ply, args )
	RangeChat( "*** " .. ply:Nick() .. " " .. args, ply:GetPos(), 550 , ply);
	return "";
end
...
function LocalOOC( ply, args )
	RangeChat( "[LocalOOC]" .. ply:Nick() .. ": " .. args, ply:GetPos(), 550 , ply);
	return "";
end
...
function Whisper( ply, args )
	RangeChat( "[WHISPER]" .. ply:Nick() .. ": " .. args, ply:EyePos(), 90 , ply);
	return "";                                                       
end
...
function Smile( ply, args )
	if( args == "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " smiles. ", ply:EyePos(), 190 , ply);
	end                                                           
	
	if( args != "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " smiles, and then says ''" .. args .. "''", ply:EyePos(), 190 , ply);
	end                                                                                        
	return "";
end
...
function Wink( ply, args )
	if( args == "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " winks. ", ply:EyePos(), 190 , ply);
	end                                                          
	if( args != "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " winks, and then says ''" .. args .. "''", ply:EyePos(), 190 , ply);
	end                                                                                       
	return "";
end
AddChatCommand( ";)", Wink );
...
function Frown( ply, args )
	if( args == "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " frowns. ", ply:EyePos(), 190 , ply);
	end                                                           
	if( args != "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " frowns, and then says ''" .. args .. "''", ply:EyePos(), 190 , ply);
	end                                                                                        
	return "";
end
...
function Cry( ply, args )
	if( args == "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " cries. ", ply:EyePos(), 190 , ply);
	end                                                          
	if( args != "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " cries, and then says ''" .. args .. "''", ply:EyePos(), 190 , ply);
	end                                                                                       
	return "";
end
AddChatCommand( ":*(", Cry );
...
function LaughOutLoud( ply, args )
	if( args == "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " laughs. ", ply:EyePos(), 190 , ply);
	end                                                          
	if( args != "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " laughs, and then says ''" .. args .. "''", ply:EyePos(), 190 , ply);
	end                                                                                       
	return "";
end
...
function Evil( ply, args )
	if( args == "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " smiles evily. ", ply:EyePos(), 190 , ply);
	end                                                                
	if( args != "") then
		RangeChat( "[Emote]" .. ply:Nick() .. " smiles evily, and then says ''" .. args .. "''", ply:EyePos(), 190 , ply);
	end                                                                                             
	return "";
end
AddChatCommand( ">: )", Evil );
