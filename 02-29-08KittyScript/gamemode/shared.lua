		...
		local class = target:GetClass();--Get the entity's class
			...
			if( class == "player" ) then --If the class is a player...
				if( player:GetPos():Distance( target:GetPos() ) < 100 ) then
					...
					PlayerMenuAdmin:AddOption( "View stickynote", function() player:ConCommand("rp_gnote " .. target:GetNWString( "RPName" )) gui.EnableScreenClicker(false) clicker=false end )
					...
					PlayerMenuAdmin:AddOption( "Explode player", function() player:ConCommand("rp_explode " .. target:GetNWString( "RPName" )) gui.EnableScreenClicker(false) clicker=false end )
					...
					PlayerMenuAdminFlags:AddOption( "Allow Vortigaunt", function() player:ConCommand("rp_allowvort " .. target:GetNWString( "RPName" )) gui.EnableScreenClicker(false) clicker=false end )
					PlayerMenuAdminFlags:AddSpacer()
					PlayerMenuAdminFlags:AddOption( "Civil Protection", function() gui.EnableScreenClicker(false) clicker=false end )
					PlayerMenuAdminFlags:AddOption( "Allow Scanner", function() player:ConCommand("rp_allowscanner " .. target:GetNWString( "RPName" )) gui.EnableScreenClicker(false) clicker=false end )
					PlayerMenuAdminFlags:AddOption( "Allow Shield Scanner", function() player:ConCommand("rp_allowshieldscanner " .. target:GetNWString( "RPName" )) gui.EnableScreenClicker(false) clicker=false end )
				end
			end
			...
			if( string.sub(class, 1, 5) == "item_" ) then  --If the class is an item...
				if( player:GetPos():Distance( target:GetPos() ) < 100 ) then
					...
					MainMenu:AddOption("Pick Up", function() player:ConCommand("rp_useitem") gui.EnableScreenClicker(false) clicker=false end )
					MainMenu:AddSpacer()
					end
			end
			...
			if( class == "prop_physics" ) then  --If the class is a prop...
				...
				submenu:AddOption( "Kick player", function() player:ConCommand("rp_kick " .. target:GetNWString("rppropspawnby") .. " ") gui.EnableScreenClicker(false) clicker=false end )
