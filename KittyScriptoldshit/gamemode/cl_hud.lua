...
local function DrawHungerDisplay()
	if( not LocalPlayer():Alive() ) then return; end
	if( LocalPlayer():GetNWInt( "hunger" ) <= 3050 ) then
		if( LocalPlayer():GetNWInt( "hunger" ) <= 2540 ) then
			local blurmul = 0;
			local cutoff = 500;
			if( LocalPlayer():GetNWInt( "hunger" ) <= 1030 ) then
				cutoff = 1200;
			end
			if( LocalPlayer():GetNWInt( "hunger" ) <= 520 ) then
				cutoff = 2000;
			end	
...
local function DrawThirstDisplay()
	if( not LocalPlayer():Alive() ) then return; end
	if( LocalPlayer():GetNWInt( "thirst" ) <= 3050 ) then
		if( LocalPlayer():GetNWInt( "thirst" ) <= 2540 ) then
			local blurmul = 0;
			local cutoff = 50;
			if( LocalPlayer():GetNWInt( "thirst" ) <= 1030 ) then
				cutoff = 120;
			end
			if( LocalPlayer():GetNWInt( "thirst" ) <= 520 ) then
				cutoff = 200;
			end	
