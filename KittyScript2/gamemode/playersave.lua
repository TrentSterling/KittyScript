...
function ExtractCertainSaveInfo( player )
	...
	if(player:GetNWString("RPNewName") == player:GetNWString( "RPName" ))then
	local pos1 = string.find( string.gsub( str, "-", "@" ), "START_CHARACTER \"" .. string.gsub( player:GetNWString( "RPName" ), "-", "@" ) .. "\"" );
	else
	local pos1 = string.find( string.gsub( str, "-", "@" ), "START_CHARACTER \"" .. string.gsub( player:GetNWString( "RPName" ), "-", "@" ) .. "\"" );
	--player:SetNWString( "RPName", player:GetNWString("RPNewName") )
	end
	if( not pos1 ) then 
...
function SaveInfo(player)
	...
	saveinfo[UniqueID .. "_inv"] = player:GetNWInt(UniqueID .. "_inv" );
	end
...
for t, s in pairs( list ) do		
	...
	if( cmd == UniqueID .. "_inv" ) then
			player:SetNWInt( UniqueID .. "_inv",  v[2])
	end
	end
...
function meta:ExtractCertainSaveInfo2( name )
	local dir = "KittyScript/playerdata/" .. self:FormatSteamID2() .. ".txt";
...
function ccChangeName(ply, cmd, args)
				...
				local info = ply:ExtractCertainSaveInfo2( ply:GetNWString( "RPName" ) );
				file.Write( "KittyScript/playerdata/" .. FormatSteamID(ply) .. ".txt", info );
				
end
concommand.Add("rp_name", ccChangeName)
