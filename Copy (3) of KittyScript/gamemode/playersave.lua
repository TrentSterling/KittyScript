...
function SaveInfo(player)
	...
	saveinfo["HUNGER"] = player:GetNWInt( "hunger" );
	saveinfo["THIRST"] = player:GetNWInt( "thirst" );
...
for t, s in pairs( list ) do		
	...
	if( cmd == UniqueID .. "_inv" ) then
		...
		elseif( cmd == "HUNGER" ) then             
			player:SetNWInt("hunger", tonumber(v[2]))
		elseif( cmd == "THIRST" ) then             
			player:SetNWInt("thirst", tonumber(v[2]))
