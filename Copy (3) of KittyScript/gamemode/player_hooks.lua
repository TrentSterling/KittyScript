...
function GM.PhysgunPickup( ply, ent )
	...
	--if( ply:GetNWInt( "adminlevel" ) > 1 ) then
		--return true 
	--end
	--end
...
ply:SetNWInt( "thirst", 10000 )
ply:SetNWInt( "thirst_add", 0 )
ply:SetNWInt( "thirst_subt", 0 )
...
ply:SetNWInt( "hunger", 10000 )
ply:SetNWInt( "hunger_add", 0 )
ply:SetNWInt( "hunger_subt", 0 )
	ply:SetNWInt("menu", 0)--MOVE THIS!
	ply:SetNWInt( "adminhiding", 0 )--MOVE THIS!
	ply:SetNWInt("togglebob", 1)--MOVE THIS!
	ply:SetNWInt("stat_sprint", 100)--MOVE THIS!
	ply:SetTeam(1);
...
function GM:PlayerSpawn( ply )
	...
	if(ply:GetNWInt("setflag") == 2) then--If true, your flag is set to scanner
		ply:ConCommand("+duck")
		ply:SetPos(Vector(2774.6907, -136.2900, 2314.0586)+ Vector(math.random( -100, 100 ),math.random( -100, 100 ), 0) )
	end
...
function RegenHealth(ply)--Resets the loop to play again with given parameters
	if(ply:GetNWInt("playeractive") == 1) then--If true, you are active
		...
		--			local trace = { }
		--			trace.start = ply:EyePos() + Vector( 0, 0, 60 );
		--			trace.endpos = trace.start + ply:GetUp() * -150;
		--			trace.endpos = trace.endpos + ( ply:GetForward() * math.random( maxdist * -1, maxdist ) );
		--			trace.endpos = trace.endpos + ( ply:GetRight() * math.random( maxdist * -1, maxdist ) );
...
ply:SetNWInt( "hunger_subt", ply:GetNWInt( "hunger_subt" ) + 0.15)

ply:SetNWInt( "thirst_subt", ply:GetNWInt( "thirst_subt" ) + 0.5)
...
function playerDies( victim, weapon, killer )
	...
	for k, v in pairs(victim:GetWeapons()) do
		if( ItemData[ v:GetClass() ] ) then 
				CreateItem( v:GetClass(), victim:GetPos() + Vector(0,0,30) );
		end
	end
	...
	for _, v in pairs( list ) do
		...
		Msg( "ITEMLIST COMPILED\n" );
		UniqueID = string.gsub( v, ".lua", "" );
		
		Msg( UniqueID .. "\n" );
		
		while (tonumber(victim:GetNWInt( UniqueID .. "_inv" )) > 0) do--make a for loop later
		
		Msg( "PLAYER HAS " .. UniqueID .. "\n" );
				CreateItem( UniqueID, victim:GetPos() + Vector(math.random( -30, 30 ),math.random( -30, 30 ), math.random( 30, 60 )) );
	
		victim:SetNWInt( UniqueID .. "_inv", victim:GetNWInt(  UniqueID .. "_inv" ) - 1 )
		end
